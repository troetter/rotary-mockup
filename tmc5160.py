...
class TMC5160:
    ...
    def read_register_value(self, reg_name):
        reg = self.regs[reg_name]
        addr = reg.address
        readable = reg.mode != Dir.W
        b = struct.pack('!BI', addr, 0)
        ...
        self.last_status, response = struct.unpack('!BI', bytes(ret))
        print(f'status: {self.last_status:02x}')
        print(f'{reg_name}: {response:08x}')
        return response
